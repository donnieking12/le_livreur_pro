...
final partnerAnalyticsProvider =
    FutureProvider.family<PartnerAnalytics, AnalyticsRequest>(
        (ref, request) async {
  return await AnalyticsService.getPartnerAnalytics(
      request.partnerId, request.period);
});
...
class _PartnerAnalyticsScreenState
  ...
  Widget build(BuildContext context) {
    final analyticsRequest =
        AnalyticsRequest(widget.partnerId, _selectedPeriod);
    final analyticsAsync =
        ref.watch(partnerAnalyticsProvider(analyticsRequest));
  ...
  Future<void> _refreshData() async {
    ref.refresh(partnerAnalyticsProvider(
        AnalyticsRequest(widget.partnerId, _selectedPeriod)));
  }
}
