...
final ordersByStatusProvider =
    FutureProvider.family<List<DeliveryOrder>, DeliveryStatus>(
        (ref, status) async {
  return await AdminService.getOrdersByStatus(status.name);
});
...
class _AdminOrdersScreenState extends ConsumerState<AdminOrdersScreen>
              ...
              IconButton(
                ...
                style: IconButton.styleFrom(
                  backgroundColor: _dateRange != null
                      ...
                      : Colors.grey[300],
                  foregroundColor:
                      _dateRange != null ? Colors.white : Colors.grey[600],
                ),
              ),
    ...
    return statsAsync.when(
      data: (stats) => Row(
        ...
        children: [
          ...
          _buildStatChip(
              'En cours'.tr(), '${stats['active']}', AppTheme.accentOrange),
          _buildStatChip(
              'Livrées'.tr(), '${stats['delivered']}', AppTheme.successGreen),
          _buildStatChip(
              'Problèmes'.tr(), '${stats['problems']}', AppTheme.errorRed),
        ],
      ),
            ...
            child: Column(
              ...
              children: [
                ...
                if (order.courierName != null)
                  _buildDetailRow('Livreur', order.courierName!),
                _buildDetailRow('Adresse de livraison',
  ...
  void _viewOrderDetails(DeliveryOrder order) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content:
            Text('Détails de ${order.orderNumber} bientôt disponibles'.tr()),
        backgroundColor: AppTheme.infoBlue,
      ),
    );
  }
  ...
  void _assignOrder(DeliveryOrder order) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content:
            Text('Attribution de ${order.orderNumber} bientôt disponible'.tr()),
        backgroundColor: AppTheme.infoBlue,
      ),
    );
  }
